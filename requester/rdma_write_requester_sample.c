...
static void rdma_write_completed_callback(struct doca_rdma_task_write *rdma_write_task,
	...
	// const struct doca_buf *src_buf = doca_rdma_task_write_get_src_buf(rdma_write_task);
	// const struct doca_buf *dst_buf = doca_rdma_task_write_get_dst_buf(rdma_write_task);
	...
	// ws 报错的源头1 ，由于每次只提交一个任务，因此执行完之后，这里会执行--导致任务数为 0.从而关闭 ctx
                                     ...
                                     char *remote_mmap_range)
{
    ...
    DOCA_LOG_INFO("提交第 %zu 块 RDMA 写任务 (大小: %zu)", chunk_idx, total_chunk_size);
    resources->num_remaining_tasks++;
    result = doca_task_submit(doca_rdma_task_write_as_task(rdma_write_task));
    if (result != DOCA_SUCCESS) {
...
// 修改后的主传输函数
static doca_error_t rdma_write_prepare_and_submit_task_chunked(struct rdma_resources *resources)
{
	TIMER_START(all);
    char *remote_mmap_range;
    size_t remote_mmap_range_len;
    ...
    // 为了测试分块传输，可设置数据重复次数而进行人为放大数据大小
    size_t test_multiplier = 1;
    ...
    for (size_t i = 0; i < test_multiplier; i++) {
        memcpy(enlarged_data + i * original_data_len, original_data_ptr, original_data_len);
    }
    ...
    size_t total_chunks = (data_len + max_data_per_chunk - 1) / max_data_per_chunk;
    
    DOCA_LOG_INFO("总文件大小: %zu, 远程缓冲区大小: %zu", data_len, remote_mmap_range_len);
    DOCA_LOG_INFO("将分为 %zu 块传输，每块最大数据: %zu", total_chunks, max_data_per_chunk);
...
//     //     DOCA_LOG_ERR("Failed to allocate memory for chunk %zu", chunk_idx);
//     //     return DOCA_ERROR_NO_MEMORY;
//     // }
...
//                             &src_buf);
//     if (result != DOCA_SUCCESS) {
//         DOCA_LOG_ERR("Failed to allocate src buffer: %s", doca_error_get_descr(result));
//         goto destroy_src_buf;
//     }
    
//     result = doca_buf_get_data(src_buf, &src_buf_data);
//     if (result != DOCA_SUCCESS) {
//         DOCA_LOG_ERR("Failed to get src buffer data: %s", doca_error_get_descr(result));
//         goto destroy_src_buf;
//     }
...
//                             &dst_buf);
//     if (result != DOCA_SUCCESS) {
//         DOCA_LOG_ERR("Failed to allocate dst buffer: %s", doca_error_get_descr(result));
//         goto destroy_dst_buf;
//     }
...
doca_error_t rdma_write_requester(struct rdma_config *cfg)
{
    ...
    DOCA_ACCESS_FLAG_RDMA_WRITE;         // ✅ 必须：允许这块内存被远端（另一个进程）写
	...
	if (init_control_client(resources.ctrl, "192.168.200.2",cfg->tcp_port) != 0) {
