...
/* ------------ 可调宏 ------------ */
#define SHM_NAME  "/dev/hugepages/doca_rdv_buf"  /* 必在 hugetlbfs 目录 */
                           ...
                           struct doca_dev *dev)
{
    ...
    if (block_offset + aligned_len > shm_state.total_size) {
        ...
        DOCA_LOG_INFO("Extended shared memory file to %zu bytes", shm_state.total_size);
    }
    ...
    for (size_t i = 0; i < required_len; i += page_sz)
        p[i] = 0;
    ...
    drc = doca_mmap_set_permissions(dm,
    ...
    if (drc != DOCA_SUCCESS) {
        DOCA_LOG_ERR("doca_mmap_enable_thread_safety: %s", doca_error_get_descr(drc));
        goto doca_fail;
    }
    ...
    DOCA_LOG_INFO("Created new block %d: offset=%zu size=%zu", 
                 block_idx, block_offset, aligned_len);
    return 0;
                              ...
                              struct my_shm_region *out)
{
    ...
    if (!name || !out || !dev || required_len == 0)
        return -EINVAL;
...
{
    if (!reg || reg->block_id < 0 || reg->block_id >= MAX_BLOCKS)
        return;
    ...
    //     block->valid = 0;
    // }else {
    //     DOCA_LOG_INFO("块 %d 仍有 %u 个使用者，保留资源", reg->block_id, block->used);
    // }

    // 始终保持块有效
    if (block->used == 0) {
        DOCA_LOG_INFO("块 %d 引用计数为0，但保持资源不释放以供后续复用", reg->block_id);
    } else {
                                             ...
                                             int block_id)
{
    ...
    double global_lock_time, block_lock_time, 
           check_reuse_time, create_time, total_operation_time;
    
    // 记录函数开始时间
    clock_gettime(CLOCK_MONOTONIC, &t1);

    if (!name || !out || !dev || required_len == 0 ||
        block_id < 0 || block_id >= MAX_BLOCKS)
        return -EINVAL;
    ...
    if (block->valid) {
        ...
        check_reuse_time = (t6.tv_sec - t5.tv_sec) + (t6.tv_nsec - t5.tv_nsec) / 1e6;
        // 实际操作时间（排除了锁等待时间）
        total_operation_time = check_reuse_time;
        
        DOCA_LOG_INFO("Block %d 复用性能分析: 全局锁等待=%.2f毫秒, 块锁等待=%.2f毫秒, "
                     "复用操作=%.2f毫秒, 纯操作时间=%.2f毫秒",
                     block_id, global_lock_time, block_lock_time, 
                     check_reuse_time, total_operation_time);

        pthread_mutex_unlock(&block->lock);
        DOCA_LOG_INFO("Reusing specified block %d, use count: %u", block_id, block->used);
        return 0;
    }
    ...
    DOCA_LOG_INFO("Block %d 创建性能分析: 全局锁等待=%.2f毫秒, 块锁等待=%.2f毫秒, "
                 "创建操作=%.2f 毫秒, 纯操作时间=%.2f毫秒",
                 block_id, global_lock_time, block_lock_time, 
                 create_time, total_operation_time);
                                     ...
                                     struct my_shm_region *out)
{
    ...
    if (best_fit_idx >= 0) {
        ...
        is_reusing = true;
        DOCA_LOG_INFO("找到合适的块 %d 复用，大小: %zu (需要: %zu)",
                     selected_idx, shm_state.blocks[selected_idx].length, required_len);
    } else if (free_idx >= 0) {
        // 没有合适的块，但有空闲槽位
    ...
    if (is_reusing) {
        ...
        DOCA_LOG_INFO("智能复用块 %d，使用计数: %u, 大小: %zu (需要: %zu), "
                     "查找耗时: %.2f毫秒, 操作耗时: %.2f毫秒",
                     selected_idx, block->used, block->length, required_len,
                     find_block_time, block_op_time);
    } else {
        // 创建新块
        ...
        DOCA_LOG_INFO("智能创建块 %d，大小: %zu (需要: %zu), "
                     "查找耗时: %.2f毫秒, 操作耗时: %.2f毫秒",
                     selected_idx, block->length, required_len,
                     find_block_time, block_op_time);
    }
...
void print_block_status(void)
{
    ...
    for (int i = 0; i < MAX_BLOCKS; i++) {
        if (shm_state.blocks[i].valid) {
            DOCA_LOG_INFO("块 %d: 有效=%d, 使用计数=%u, 大小=%zu, 地址=%p",
                         i, shm_state.blocks[i].valid, shm_state.blocks[i].used,
