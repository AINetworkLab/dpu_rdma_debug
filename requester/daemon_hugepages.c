...
/* ===== 修改后的宏 ===== */
#define SHM_NAME  "/dev/hugepages/doca_rdv_buf"   /* 一定在 hugetlbfs 里 */
...
static void handle(int s){ (void)s; cleanup(); }
...
int main(int argc,char *argv[])
{
    ...
    /* --- HugePages 余量检查 --- */
    {
        FILE *f=fopen("/proc/meminfo","r"); long free_hp=0; char line[128];
    ...
    daemonize();
    signal(SIGTERM,handle); signal(SIGINT,handle);
    ...
    if(fp){ fprintf(fp,"%s %lu\n", SHM_NAME, SHM_SIZE); fclose(fp); }
